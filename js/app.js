        ...
        let gotoBlock = (targetBlock, noHeader = false, speed = 1e3, offsetTop = 0) => {
            ...
            if (targetBlockElement) {
                ...
                if ("undefined" !== typeof smooth_scroll_polyfills_min) (new smooth_scroll_polyfills_min).animateScroll(targetBlockElement, "", options); else {
                    let targetBlockElementPosition = targetBlockElement.getBoundingClientRect().top + window.pageYOffset;
...
***************************************************************************** */
        ...
        var links = document.querySelectorAll(".menu__link, .sub-menu__link");
        var sections = document.querySelectorAll(".page__section");
        window.onscroll = function() {
            var scroll_position = window.scrollY;
            if (scroll_position > 250) {
                header.style.backgroundColor = "rgba(84, 100, 112, .8)";
                document.getElementById("bgm-1").style.backgroundColor = "rgba(84, 100, 112, .8)";
                document.getElementById("bgm-2").style.backgroundColor = "rgba(84, 100, 112, .8)";
                document.getElementById("bgm-3").style.backgroundColor = "rgba(84, 100, 112, .8)";
            } else {
                header.style.backgroundColor = "transparent";
                document.getElementById("bgm-1").style.backgroundColor = "transparent";
                document.getElementById("bgm-2").style.backgroundColor = "transparent";
                document.getElementById("bgm-3").style.backgroundColor = "transparent";
            }
            var current = "welcome";
            sections.forEach((section => {
                const sectionTop = section.offsetTop;
                if (window.scrollY >= sectionTop - 500) current = section.getAttribute("id");
            }));
            links.forEach((item => {
                item.classList.remove("active");
                if (item.href.includes(current)) item.classList.add("active"); else item.classList.remove("active");
            }));
        };
        window["FLS"] = true;
        isWebp();
